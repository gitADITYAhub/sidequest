...
export class QuestService {
    ...
    async seedQuests(): Promise<string> {
        const count = await this.questTemplateRepository.count();
        if (count > 0) {
            return `Database already has ${count} quests. Seeding skipped.`;
        }
        for (const quest of this.SEED_QUESTS) {
            await this.questTemplateRepository.save(quest);
        }
        return `Successfully seeded ${this.SEED_QUESTS.length} quests!`;
    }
    ...
    async rollQuest(): Promise<QuestTemplate> {
        ...
        if (quests.length === 0) {
            ...
            return {
                id: 0,
                description: 'Welcome Query! Take a selfie to start your journey.',
                difficulty: 'Easy',
                type: 'Solo',
                tags: ['intro'],
                xp: 10,
